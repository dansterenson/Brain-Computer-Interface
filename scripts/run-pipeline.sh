...
sudo docker build -f Noesis/server/Dockerfile -t server .
sudo docker build -f Noesis/saver/Dockerfile -tsaver .
sudo docker build -f Noesis/api/Dockerfile -t api .
sudo docker build -f Noesis/gui/Dockerfile -t gui .
sudo docker build -f Noesis/parsers/Dockerfile -t pose --build-arg PARSER_NAME=pose .
sudo docker build -f Noesis/parsers/Dockerfile -t color_image --build-arg PARSER_NAME=color_image .
sudo docker build -f Noesis/parsers/Dockerfile -t depth_image --build-arg PARSER_NAME=depth_image .
sudo docker build -f Noesis/parsers/Dockerfile -t feelings --build-arg PARSER_NAME=feelings .
